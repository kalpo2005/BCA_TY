...
DECLARE dept_name department.deptName %type := '&dept_name';
CURSOR myCursor IS
SELECT e.*
FROM empdata e,
	department d
WHERE e.deptId = d.deptId
	AND d.deptName = dept_name
	AND e.joiningDate <= ADD_MONTHS(SYSDATE, -60);
...
DBMS_OUTPUT.PUT_LINE(
	memoryVariable.empId || ' ' || memoryVariable.empName || ' ' || memoryVariable.deptId
);
END LOOP;
CLOSE myCursor;
END;
